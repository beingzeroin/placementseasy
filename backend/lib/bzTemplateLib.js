var BZTemplateModel = require('../models/bzTemplateModel');

exports.getAllTemplates = function(req, res) {
    BZTemplateModel.find(req.query).populate('users').exec(function(err, dbItems) {
        //console.log(dbItems);
        if (err)
...
exports.createTemplate = function(req, res) {
    ...
    var ti = new BZTemplateModel(req.body);
...
exports.getTemplate = function(req, res) {
    ...
    BZTemplateModel.findById(req.params.id, function(err, cObj) {
        // Return Object
        if (err) {
            BZTemplateModel.findOne({ 'title': req.params.title }, function(error, tObj) {
                if (error)
